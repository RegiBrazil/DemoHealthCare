 ...
 * successful DemoHealthCare build. The result of the script is the generation of a build list text
 ...
 * DemoHealthCare build.groovy script.
 *
 * If the --buildHash option is omitted or if no previous successful DemoHealthCare build exists with a
 * buildHash property, the contents of DemoHealthCare/build/files.txt will be copied to ${workDir}/buildList.txt
 * resulting in a full build.
 *
 * usage:  impacts.groovy [options]
 *
 *  **NOTE - DemoHealthCare/build/build.properties will be loaded (if exists) at script startup.
...
// if buildHash argument omitted, then just copy DemoHealthCare/build/files.txt to buildlist and exit
...
// if no lastBuildHash, then just copy DemoHealthCare/build/files.txt to buildlist and exit
