...
@Field def packageUtils= loadScript(new File("utilities/PackageUtilities.groovy"))
@Field String hashPrefix = ':githash:'
@Field RepositoryClient repositoryClient
@Field userBuildComp = false // Allow to use the old IDz/ZOD implementation of user build
...
options:
	...
	cli.c(longOpt:'currentChangeBuild', 'Flag indicating to build only programs in the current commit.')
	...
	// web application credentials (overrides properties in build.yaml)
	...
	cli.cc(longOpt:'codecov', 'Flag to indicate a test with codecoverage')
...
def populateBuildProperties(String[] args) {
	...
	if (opts.w) props.workspace = opts.w  else if  ( userBuildComp ) props.workspace = opts.srcDir
	...
	props.load(new File("${buildConf}/build.yaml"))
	
	// If DBB does not support yaml file properties fall back to properties file
	if ( ! props.applicationConfRootDir ) {
		props.load(new File("${buildConf}/build.properties"))
		println("*? Warning this version of DBB does not support yaml file properties fall back to build.properties file ..")
	}
	...
	def file = new File("${appConf}/application.properties")
	
	if ( ! file.exists() ) {
		println "*? Warning use application default location:"
		println "*?    appConf from configuration = ${appConf}"
		appConf = "${props.workspace}/${props.application}/application-conf"
		println "*?    appConf default= ${appConf}"
		file = new File("${appConf}/application.properties")
	}
	...
	if (opts.c) props.currentChangeBuild = 'true'
	...
	if ( ! props.enableTopicBranchBuild || props.enableTopicBranchBuild.toBoolean() ) {
		props.topicBranchBuild = (props.applicationCurrentBranch.equals(props.mainBuildBranch)) ? null : 'true'
	} else {
		props.topicBranchBuild = null
	}
	...
	// NAZARE : We only keep one build group for all PRs. Can't test CHANGE_ID env var beacause of possible use of DBB daemon.
	def isPullResquest = false
	def idx = props.applicationBuildGroup.indexOf("PR-")
	if ( idx > 0 ) {
		// GitHub
		props.applicationBuildGroup = "${props.application}-pr" as String
		isPullResquest = true
	} else {
		// GitLab
		def exp =  /.*\-[0-9]*/
		if ( props.applicationBuildGroup ==~ exp) {
			props.applicationBuildGroup = "${props.application}-pr" as String
			isPullResquest = true
		}
	}
	println "** DBB build group name: $props.applicationBuildGroup"
	...
	if ( isPullResquest == true )
		props.applicationCollectionName = "${props.application}-pr" as String
	props.applicationOutputsCollectionName = "${props.applicationCollectionName}-outputs" as String
	if (!props.userBuild) println "** DBB collection name: $props.applicationCollectionName"
	
	// NAZARE : With a specific folder we need to have this information in Jenkins
	//if (userBuildComp)
    ...
    // Other options
	if (opts.repoUrl) props.repoUrl = opts.repoUrl
	if (opts.buildNb) props.buildNumber = opts.buildNb
...
def createBuildList() {
	...
	// check if impact build
		...
		if (repositoryClient) {
			buildSet = impactUtils.createImpactBuildList(repositoryClient, false)
		}
		else {
			println "*! Impact build requires a repository client connection to a DBB web application"
		}
	}
	// check if current change  build
	else if (props.currentChangeBuild) {
		println "** --currentChangeBuild option selected. $action modified programs for application ${props.application} "
		buildSet = impactUtils.createImpactBuildList(repositoryClient, true)
	}	
	...
	if (!props.impactBuild && !props.userBuild && !props.currentChangeBuild) {
